...
def update_charts(*selected_values):
    ...
    df_summary_latest = df_summary_latest[df_summary_latest['datestamp'] >= pd.to_datetime(pd.Timestamp.utcnow().tz_localize('UTC') - pd.DateOffset(days=config.get('stale_metric',2)))]
